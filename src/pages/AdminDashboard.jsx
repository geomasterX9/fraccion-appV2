            ...
            {['todos', 'al-corriente', 'con-adeudo'].map((f) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                  filtro === f ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'
                }`}
              >
                {f.replace('-', ' ')}
              </button>
            ...
            {propiedadesFiltradas.map(p => (
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className={`size-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-lg transition-transform group-hover:scale-110 ${
                    p.tieneAdeudo ? 'bg-red-50 text-red-500 shadow-red-500/10' : 'bg-green-50 text-green-500 shadow-green-500/10'
                  }`}>
                    {p.numero_casa}
                  </div>
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${
                    p.tieneAdeudo ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'
                  }`}>
